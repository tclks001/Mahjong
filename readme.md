...
# 游戏玩法
在扁形瓶（长6cm，宽3.6cm，厚0.2cm）中填满流沙油（略微粘稠的液体）<br>
...
```mermaid
graph TD
    UI[UI模块 ImGui] -->|鼠标/按钮事件<br>（重力方向/容器姿态）| PM[物理模块]
...
- 依赖：仅依赖UI模块获取当前状态<br>
  在物理模拟时假设容器是静止的，用偏转重力与外界施力对液体造成的速度场驱动粒子运动<br>
  用本帧和上一帧之间的位置与偏转差值计算出受力情况，通过偏转计算出偏转后的重力方向<br>
  输出粒子位置数组供渲染模块读取。
...
flowchart TD
    ...
    Input2[本次移动后的坐标vec3<br>姿态四元数quat] -.-> Step1
...
> 用Visual Studio，在NuGet里直接安装glfw、glew、glm。<br>
> stb_image下载后直接放在项目中，PhysX5在GitHub下载源码，并在StaticDebug模式下编译。<br>
...
> 使用空间哈希网格，网格边长取2倍粒子直径，实现O(n)更新<br>
> 把流体当做场，因为容器非常扁（厚度仅长宽的1/20）<br>
> 不考虑沿容器厚度方向的流体阻力<br>
> 用单层纳维 - 斯托克斯方程计算流体的流动速度场（二维）<br>
> 用拖曳力模型把速度场施加到粒子上。<br>
> 越靠近容器壁附近越增加额外的阻尼，模拟“挤压液膜”效应<nr>
...
> 构建并实现物理模型。<br>
> 用PBD实现粒子效果。<br>
> 粒子间和粒子与容器壁的内力作用先不考虑<br>
> 每次更新考虑外力（重力、离心力、惯性力、地转偏向力、流体力）作用下的加速度<br>
> 根据力更新速度，根据速度更新位置。<br>
> 使用空间哈希网格，在相邻粒子距离小于直径、或粒子距离容器壁小于半径时将其略微推开<br>
> 进行几次迭代后得出最终位置，并根据位置偏移量计算这一帧的确切速度。<br>

> 实现了简单的渲染和空间哈希网格，但是渲染非常简单，物理引擎效果也很差<br>

> 20260422<br>
> 构建摩擦力效果<br>
> 基于速度的摩擦力。在粒子过近相斥时，同时修正它们的相对速度<br>
> 使相对速度减少一个固定值，相对速度减少到一定阈值下时归零。<br>
> 优化空间哈希，不用哈希表，改用固定分配的vector
> 把空间分成
> ```
> ceil(containerWidth/cellSize)*ceil(containerDepth/cellSize)*ceil(containerHeight/cellSize)
> ```
> 的小方格，任何一个方格都用一个`vector<int>`记录其中的粒子编号
> 还有个问题，少量PBD迭代并不能彻底解决粒子间距太近的问题，这个要怎么办？
> 用密度约束，先计算粒子的所有邻居对他的合斥力，再一次修改，避免各管各的。